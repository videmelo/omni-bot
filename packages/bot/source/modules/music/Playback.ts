...
export class Playback extends EventEmitter {
  ...
  public async getAudioStream(track: Track, seek: number = 0) {
    ...
    if (track.cached) {
      if (track.streamable) {
        ...
        if (streamable) {
          streamable.on('error', (err) =>
            stream.destroy(typeof err === 'string' ? new Error(err) : (err)),
          );
          streamable.pipe(stream);
        } else {
      ...
      if (streamable) {
        streamable.on('error', (err) =>
          stream.destroy(typeof err === 'string' ? new Error(err) : (err as Error)),
        );
        streamable.pipe(stream);
      } else {
    ...
    const opus = fluent({ source: stream })
      ...
      .on('error', (err: unknown) => {
        ...
        logger.error(`[Playback: ${this.id}] ffmpeg error in getAudioStream:`, error);
  ...
  public getPosition() {
    return (this.audioresource?.playbackDuration ?? 0) + this.position;
  }
