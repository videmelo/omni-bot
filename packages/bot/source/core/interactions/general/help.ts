    ...
    try {
      ...
      if (input) {
        ...
        if (command) {
          const usage = command.usage
            ? `\n**Usage**\n</${command.name}:${command.commandName}> \`${command.usage}\`\n`
            : '';
          const exemple = command.exemple
            ? `\n**Exemple**\n</${command.name}:${command.commandName}> \`${command.exemple}\``
            : '';
      ...
      const list = commands
        ...
        .map((command) => {
          ...
          return `</${command.name}:${command.commandName}>${usage}\n${command.description}`;
        });
      ...
      const embeds = pages.map((message, index) => {
        return client.embed.new({
          ...
          thumbnail: `https://cdn.discordapp.com/avatars/${client.user!.id}/${client.user!.avatar}.jpeg`,
          description: help
            ? `Want info about a specific command?\n</${help.name}:${help.id}> \`[command]\``
            : 'Want info about a specific command? Use `/help [command]`',
