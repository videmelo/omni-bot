...
export default class Interactions {
   ...
   async load() {
      const folders = fs.readdirSync('./source/core/interactions').filter((file) => fs.statSync(`./source/core/interactions/${file}`).isDirectory());
      try {
         logger.async('Started loading interactions:');
         await Promise.all(
            folders.map(async (folder) => {
               const files = fs.readdirSync(`./source/core/interactions/${folder}`).filter((file) => file.endsWith('.ts'));
               await Promise.all(
