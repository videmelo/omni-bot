...
export default class Events {
   ...
   async load() {
      const folders = fs.readdirSync('./source/core/events').filter((file) => fs.statSync(`./source/core/events/${file}`).isDirectory());
      try {
         ...
         await Promise.all(
            folders.map(async (folder) => {
               const files = fs.readdirSync(`./source/core/events/${folder}`).filter((file) => file.endsWith('.ts'));
               await Promise.all(
