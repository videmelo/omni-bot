          ...
          <div className="flex overflow-x-auto scrollbar-none gap-3 bg-black bg-opacity-40 hover:bg-opacity-50 rounded-2xl group">
            ...
            <div className="flex w-full min-w-0 justify-between">
              <div className="flex flex-col min-w-0 gap-1 p-4 pb-0">
                <div className="font-poppins font-bold text-3xl truncate min-w-0 w-full">
                  {top?.name}
                </div>
                ...
                <div className="font-poppins flex gap-1 font-normal text-base text-[#B3B3B3]">
                  ...
                  <span className="whitespace-nowrap">
                    {top.artists ? top.artists[0].name : (top.artist?.name ?? null)}
                  </span>
                </div>
              </div>
            ...
            <div className="flex flex-col w-full">
              {tracks.slice(0, 4).map(({ name, artist, icon, id }) => (
                ...
                >
                  ...
                  <div className="w-full min-w-0">
                    <div className="font-medium text-sm font-poppins text-white truncate">
                      {name}
                    </div>
      ...
      <div className="flex flex-col gap-5">
        {tracks.length > 0 ? (
          <Section
            data={tracks}
            title="Tracks"
            onClick={(_event, item) => handleItemClick(item as unknown as Track)}
            onAppend={(event, track) => {
