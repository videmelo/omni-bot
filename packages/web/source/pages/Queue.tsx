...
export default function Page() {
   ...
   const { track, playing, queue, next, paused, timer, palette, cover, volume, voice } = state;
   const [artist, setArtist]: any = useState(null);
   useEffect(() => {
      const dark = new Vibrant(track?.artist.icon);
      dark.getPalette().then((colors) => {  
         setArtist(colors);
      });
   }, [track]);
               ...
               <div className={`scrollbar-none w-full h-full flex-col flex rounded-3xl`}>
                  <div className={`flex flex-col items-center w-full min-w-0 duration-700 mb-9`}>
                     ...
                     <div className="flex flex-col gap-1 items-center w-full min-w-0 p-8 justify-between">
                        <div className="font-semibold text-center w-full text-3xl truncate font-poppins">{track.name}</div>
                  ...
                  <div className="flex bg-[#1a1a1a] max-xl:flex-col-reverse gap-5 rounded-3xl p-6 mt-0 -m-7">
                           ...
                           <div className="flex gap-4 flex-col">
                              ...
                              >
                                 <div className="relative ">
                                    <img className="h-full w-auto object-cover z-0" src={track.artist.icon} alt="" />
                                    <div
                                 ...
                                 <div className="flex flex-col justify-center -mt-12 z-20 p-8">
                                    ...
                                    <span className="capitalize w-fit bg-white p-[2px] px-2 rounded-md text-sm bg-opacity-10">Artist</span>
                                 </div>
                              </div>
                           </div>
                           <div className="flex w-full flex-auto  h-fit rounded-2xl p-3 xl cursor-pointer hover:scale-[1.02] bg-black bg-opacity-60 transition-all">
                              <div className="flex w-full items-center gap-3 ">
                                 ...
                                 <div className="w-full min-w-0">
                                    <div className="font-medium text-lg font-poppins text-white truncate">{track.album.name}</div>
                                    <div className="flex gap-2">
                                       <span className="capitalize w-fit bg-white p-[2px] px-2 rounded-md text-sm bg-opacity-10">Album</span>
                                       <div className="font-normal text-base font-poppins text-[#B3B3B3]">{track.artist.name}</div>
                                    </div>
                                 </div>
                              </div>
                           </div>
                     ...
                     <div className="flex flex-col gap-3 flex-auto">
                        ...
                        <div className="flex flex-col gap-3 p-4 rounded-3xl cursor-pointer hover:scale-[1.02] transition-all bg-black bg-opacity-60">
                           <span className="capitalize w-fit bg-white p-[2px] px-2 rounded-md text-sm bg-opacity-10">Next</span>
                           <div className="flex items-center gap-3">
                              <img className="h-20 w-20 rounded-xl" src={next?.icon} alt="" />
                              <div className="w-full min-w-0">
                                 <div className="font-medium text-xl font-poppins w-full text-white truncate">{next?.name}</div>
                                 <div className="font-normal text-lg font-poppins text-[#B3B3B3]">{next?.artist.name}</div>
                              </div>
                           </div>
                        </div>
                     </div>
